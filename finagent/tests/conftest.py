...
def migrated_db_url(pg_container: PostgresContainer) -> Generator[str, None, None]:
    ...
    psycopg_url = url.replace("postgresql+asyncpg", "postgresql+psycopg", 1)
    cfg = Config(str(Path("alembic.ini").resolve()))
    original_db_url = os.environ.get("MIGRATION_DATABASE_URL")
    os.environ["MIGRATION_DATABASE_URL"] = psycopg_url
    try:
        ...
        if original_db_url is None:
            ...
            os.environ["MIGRATION_DATABASE_URL"] = original_db_url
